...
class KNN:
    ...
    def predict_single(self, x_new):
        ...
        dist_list = [
            (-np.linalg.norm(x_new - self.X_train[i], ord=2), self.y_train[i], i)
        ...
        for i in range(self.k, self.X_train.shape[0]):
            dist_i = (
                -np.linalg.norm(x_new - self.X_train[i], ord=2),
                self.y_train[i],
                i,
            )
    ...
    def predict_many(self, X_new):
        ...
        with futures.ProcessPoolExecutor(max_workers=4) as executor:
            # 建立多进程任务
            tasks = [
                executor.submit(self.predict_single, X_new[i])
